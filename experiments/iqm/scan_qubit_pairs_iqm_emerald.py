...
#!/usr/bin/env python3
"""
Scan qubit pairs for IQM Emerald (54-qubit square lattice) using exact Leggett test logic.
Test C(a2, b2) and C(a2, b2') correlations with φ = 30°.

IQM Emerald specifications:
- 54 qubits (superconducting transmon)
- Square lattice topology (9x6 grid with nearest-neighbor connectivity)
- Native gates: CZ, single-qubit rotations
- Typical fidelities: ~99.9% 1Q, ~99% 2Q, ~98% readout

This version simulates per-qubit noise variation to model what real hardware
would show. On real IQM hardware, different qubit pairs would have different
error rates based on their calibration state.

NOTE: The Braket LocalSimulator doesn't have qubit-specific noise models.
This script simulates heterogeneous noise by varying error rates per-pair
based on a seeded random distribution typical of superconducting qubits.
"""
...
def get_iqm_emerald_coupling_map():
    """
    Generate the coupling map for IQM Emerald's 54-qubit square lattice.

    The topology is a 9x6 grid (54 qubits) with nearest-neighbor connectivity.
    Qubit numbering: row-major order, q[i] = row * 6 + col
    """
    rows, cols = 9, 6  # 54 qubits total
    edges = []
...
def generate_qubit_error_rates(num_qubits=54, seed=42):
    """
    Generate realistic per-qubit error rates based on typical IQM calibration data.
    ...
    # Base error rates (IQM Emerald typical)
    base_q1_error = 0.001  # 0.1% 1Q error
    base_q2_error = 0.01   # 1% 2Q error
    base_readout = 0.02    # 2% readout error

    # Generate variation (log-normal distribution is typical for gate errors)
    q1_errors = {}
    readout_errors = {}
    ...
    q2_errors = {}
    edges = get_iqm_emerald_coupling_map()
...
def create_single_correlation_circuit(a_vec, b_vec):
    ...
    Always uses qubits 0 (Alice) and 1 (Bob) since LocalSimulator
    doesn't support arbitrary qubit indices meaningfully.
    """
    circ = Circuit()
...
def add_iqm_noise_model(circ, q1_error=0.001, q2_error=0.01, readout_error=0.02):
    """
    Add IQM-style noise to a circuit with specified error rates.
    """
    noisy_circ = circ.copy()

    # Apply depolarizing noise after single-qubit gates
    ...
    # Apply two-qubit depolarizing noise after CNOT
    noisy_circ.apply_gate_noise(
...
def test_qubit_pair(q1, q2, q1_errors, q2_errors, readout_errors, use_noise=True, phi_deg=30, num_shots=500):
    ...
    Uses per-pair noise rates from the error dictionaries.
    ...
    for b_vec, label in [(b2, 'b2'), (b2_prime, "b2'")]:
        ...
        if use_noise:
            circ = add_iqm_noise_model(circ, avg_q1_error, q2_error, avg_readout)
        ...
        # Calculate correlation (qubits are always 0,1)
        correlation = 0.0

        for bitstring, count in counts.items():
            padded = bitstring.zfill(2)
            alice_bit = int(padded[1])  # q0
            bob_bit = int(padded[0])    # q1
...
def scan_coupled_pairs(use_noise=True, num_shots=500, phi_deg=30, max_pairs=None):
    """Scan coupled qubit pairs on IQM Emerald topology with simulated noise heterogeneity."""
    edges = get_iqm_emerald_coupling_map()
    ...
    print(f"Testing {len(unique_pairs)} qubit pairs on IQM Emerald (54-qubit square lattice)")
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("SCANNING QUBIT PAIRS - IQM EMERALD (54-qubit square lattice)")
    print("="*70)
    ...
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'qubit_pair_scan_iqm_emerald_phi{int(phi_deg)}{noise_suffix}.json')

    with open(output_file, 'w') as f:
        json.dump({
            'device': 'IQM Emerald (emulator)',
            'topology': '54-qubit square lattice (9x6)',
