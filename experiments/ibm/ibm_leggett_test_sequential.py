...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    if use_ibm:
        ...
        if dry_run:
            print(f"  DRY RUN: Would submit job here.")
            # Print physical qubit mapping
            layout = qc_transpiled.layout
            if layout:
                print(f"\n  Physical qubit mapping (virtual -> physical):")
                # Get the final layout after routing
                final_layout = layout.final_index_layout()
                physical_qubits = final_layout
                print(f"    All physical qubits: {physical_qubits}")
                # Show pairs (logical qubits 0-1 are pair 0, 2-3 are pair 1, etc.)
                print(f"\n  Physical qubit pairs used:")
                labels = ["C(a1,b1) +phi", "C(a1,b1') +phi", "C(a2,b2) +phi",
                          "C(a2,b2') +phi", "C(a3,b3) +phi", "C(a3,b3') +phi",
                          "C(a1,b1) -phi", "C(a1,b1') -phi", "C(a2,b2) -phi",
                          "C(a2,b2') -phi", "C(a3,b3) -phi", "C(a3,b3') -phi"]
                for pair_idx in range(12):
                    alice_logical = 2 * pair_idx
                    bob_logical = 2 * pair_idx + 1
                    alice_phys = physical_qubits[alice_logical]
                    bob_phys = physical_qubits[bob_logical]
                    print(f"    Pair {pair_idx} ({labels[pair_idx]}): logical ({alice_logical},{bob_logical}) -> physical ({alice_phys},{bob_phys})")
            return None
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_24qb.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_24qb.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_24qb.json')
