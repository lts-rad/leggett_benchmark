...
ALTERNATE LAYOUT: Uses top 12 pairs from 64-pair analysis ranked by distance from QM theory.
These are the pairs that performed best in actual hardware tests.

Top 12 pairs from 64-pair analysis (by avg distance from QM theory):
1.  (72, 73)   AvgΔ=0.017
2.  (86, 87)   AvgΔ=0.022
3.  (20, 21)   AvgΔ=0.023
4.  (98, 111)  AvgΔ=0.026
5.  (12, 13)   AvgΔ=0.026
6.  (122, 123) AvgΔ=0.030
7.  (24, 25)   AvgΔ=0.032
8.  (144, 145) AvgΔ=0.032
9.  (97, 107)  AvgΔ=0.033
10. (68, 69)   AvgΔ=0.034
11. (19, 35)   AvgΔ=0.036
12. (38, 49)   AvgΔ=0.036
...
# Top 12 pairs from 64-pair analysis (ranked by distance from QM theory)
# Flattened for layout: [pair1_q0, pair1_q1, pair2_q0, pair2_q1, ...]
ALTERNATE_LAYOUT = [
    72, 73,    # Pair 1: AvgΔ=0.017 (BEST)
    86, 87,    # Pair 2: AvgΔ=0.022
    20, 21,    # Pair 3: AvgΔ=0.023
    98, 111,   # Pair 4: AvgΔ=0.026
    12, 13,    # Pair 5: AvgΔ=0.026
    122, 123,  # Pair 6: AvgΔ=0.030
    24, 25,    # Pair 7: AvgΔ=0.032
    144, 145,  # Pair 8: AvgΔ=0.032
    97, 107,   # Pair 9: AvgΔ=0.033
    68, 69,    # Pair 10: AvgΔ=0.034
    19, 35,    # Pair 11: AvgΔ=0.036
    38, 49,    # Pair 12: AvgΔ=0.036
]
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    if use_ibm:
        ...
        print(f"  Using ALTERNATE 24-qubit layout (top 12 from 64-pair analysis):")
        print(f"    {ALTERNATE_LAYOUT}")

        qc_transpiled = transpile(qc, backend=backend, optimization_level=3, initial_layout=ALTERNATE_LAYOUT)
        ...
        if use_noise_model:
            ...
            print(f"  Using ALTERNATE 24-qubit layout: {ALTERNATE_LAYOUT}")
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_24qb_ALTERNATE_10k.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_24qb_ALTERNATE_10k.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_24qb_ALTERNATE_10k.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: Sequential Jobs with 24 Qubits (IBM Quantum)")
    print("USING ALTERNATE LAYOUT: TOP 12 PAIRS FROM 64-PAIR ANALYSIS")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
    ...
    print(f"\n  ALTERNATE LAYOUT (top 12 pairs from 64-pair hardware analysis):")
    pairs = [(ALTERNATE_LAYOUT[i], ALTERNATE_LAYOUT[i+1]) for i in range(0, len(ALTERNATE_LAYOUT), 2)]
    for i, pair in enumerate(pairs):
        print(f"    Pair {i+1}: {pair}")
    print(f"\n  Error mitigation: {'enabled' if enable_error_mitigation else 'disabled'}")
