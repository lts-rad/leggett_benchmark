...
#!/usr/bin/env python3
"""
Leggett Inequality Test using IBM Quantum - 64 Pairs (128 Qubits)
Based on arXiv:0801.2241v2

Uses 64 entangled pairs simultaneously (~5.3x redundancy).
Submits ALL angle jobs concurrently.
Uses explicit initial_layout to avoid SWAP gates.
"""
...
from leggett import create_leggett_circuit_64_pairs, extract_correlations_from_counts_64_pairs, calc_leggett_for_angle

# 64 non-overlapping adjacent pairs on IBM Pittsburgh (from coupling map)
ADJACENT_PAIRS_64 = [
...
# Flatten to initial_layout [alice0, bob0, alice1, bob1, ...]
INITIAL_LAYOUT_64 = []
for a, b in ADJACENT_PAIRS_64:
    INITIAL_LAYOUT_64.extend([a, b])


def submit_job_for_angle(phi_deg, num_shots, backend, sampler):
    """
    ...
    # Create 64-pair circuit
    qc = create_leggett_circuit_64_pairs(phi_rad)
    print(f"    Circuit: {qc.num_qubits} qubits, depth {qc.depth()}")

    # Transpile with explicit initial_layout (no SWAPs needed)
    qc_transpiled = transpile(qc, backend=backend, optimization_level=3, initial_layout=INITIAL_LAYOUT_64)
    print(f"    Transpiled: depth {qc_transpiled.depth()}, CZ gates: {qc_transpiled.count_ops().get('cz', 0)}")
...
def wait_for_job(phi_deg, job, job_id, num_shots):
    ...
    correlations_pos, correlations_neg = extract_correlations_from_counts_64_pairs(counts, num_shots)

    # Calculate results
    result_pos = calc_leggett_for_angle(correlations_pos, phi_rad)
    result_neg = calc_leggett_for_angle(correlations_neg, -phi_rad)

    print(f"    +{abs(phi_deg)}°: L₃={result_pos['L3']:.4f}, bound={result_pos['bound']:.4f}, violated={result_pos['violated']}")
    print(f"    -{abs(phi_deg)}°: L₃={result_neg['L3']:.4f}, bound={result_neg['bound']:.4f}, violated={result_neg['violated']}")
...
def main():
    ...
    if output_file is None:
        ...
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_64pairs_{timestamp}_10k.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_sim_64pairs_{timestamp}_10k.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: 64 Pairs (128 Qubits)")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
    ...
    print(f"  Circuit size: 128 qubits (64 singlet pairs)")
    print(f"  Redundancy: ~5.3 copies of each correlation")
    print(f"  Initial layout: 64 adjacent pairs (no SWAPs)")
    ...
    if use_ibm:
        ...
        if enable_error_mitigation:
            ...
            print(f"\n  Error mitigation: twirling + DD (XY4)")

        sampler = Sampler(backend, options=options)

        if dry_run:
            print("\n*** DRY RUN: Testing transpilation only ***")
            qc = create_leggett_circuit_64_pairs(np.radians(30))
            print(f"  Original: {qc.num_qubits} qubits, depth {qc.depth()}")
            qc_t = transpile(qc, backend=backend, optimization_level=3, initial_layout=INITIAL_LAYOUT_64)
            print(f"  Transpiled: depth {qc_t.depth()}, CZ gates: {qc_t.count_ops().get('cz', 0)}")
        ...
        for angle in test_angles:
            job_info = submit_job_for_angle(angle, num_shots, backend, sampler)
            if job_info:
        ...
        for angle in test_angles:
            ...
            print(f"    +{angle}°: L₃={result_pos['L3']:.4f}")
            print(f"    -{angle}°: L₃={result_neg['L3']:.4f}")

            results.append({
                'phi_deg': angle, 'phi_rad': phi_rad, **result_pos,
                'job_id': 'simulator', 'num_shots': num_shots, 'num_pairs': 64,
                'timestamp': datetime.now().isoformat()
            })
            results.append({
                'phi_deg': -angle, 'phi_rad': -phi_rad, **result_neg,
                'job_id': 'simulator', 'num_shots': num_shots, 'num_pairs': 64,
                'timestamp': datetime.now().isoformat()
            })
    ...
    print(f"\nTotal angles: {len(results)}, Violations: {violations}")

    print(f"\n{'Angle':>8} {'L₃(exp)':>10} {'Bound':>10} {'Status':>12}")
    print("-"*50)
    for r in results:
        status = "VIOLATION" if r['violated'] else "-"
        print(f"{r['phi_deg']:>+8.1f}° {r['L3']:>10.4f} {r['bound']:>10.4f} {status:>12}")
