...
Each job measures 12 correlations (6 for +φ, 6 for -φ) using 12 independent singlet pairs = 24 qubits total.
"""
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    if use_ibm:
        ...
        # Use best 12 non-overlapping pairs from qubit pair scan
        # Pairs: (67,68), (29,30), (54,55), (93,92), (109,110), (2,3), (13,14), (112,113), (153,152), (75,74), (94,95), (107,97)
        #best_layout = [67, 68, 29, 30, 54, 55, 93, 92, 109, 110, 2, 3, 13, 14, 112, 113, 153, 152, 75, 74, 94, 95, 107, 97]
        #best_layout =  [86, 87, 33, 39, 80, 81, 114, 115, 11, 12, 37, 45, 21, 22, 97, 107, 148, 149, 78, 89, 74, 75, 84, 85]
        #best_layout = [81, 82, 86, 87, 99, 115, 72, 73, 60, 61, 147, 148, 152, 153, 40, 41, 46, 47, 84, 85, 100, 101, 97, 107]
        #crazy t1t2 guess:
        #best_layout = [60, 61, 147, 148, 72, 73, 85, 86, 40, 41, 99, 115, 100, 101, 80, 81, 130, 131, 11, 12, 152, 153, 97, 107]

        best_layout = [86, 87, 113, 114, 99, 115, 60, 61, 37, 45, 46, 47, 11, 18, 147, 148, 100, 101, 40, 41, 97, 107, 84, 85]


        print(f"  Using best 24-qubit layout from scan: {best_layout}")
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_24qb_BEST_LAYOUT_newbits.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_24qb_BEST_LAYOUT_newbits.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_24qb_BEST_LAYOUT_newbits.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: Sequential Jobs with 24 Qubits (IBM Quantum)")
    print("USING BEST 12 NON-OVERLAPPING PAIRS FROM QUBIT PAIR SCAN")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
