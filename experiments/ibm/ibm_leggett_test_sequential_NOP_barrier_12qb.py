...
from leggett import create_leggett_circuit_for_angle_six_barrier_NOP, extract_correlations_from_counts_six, extract_correlations_from_counts, calc_leggett_for_angle
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    print(f"\n{'='*70}")
    print(f"Running  angles φ = ±{(phi_deg):.1f}° ({num_shots} shots, 12 qubits)")
    print(f"{'='*70}")

    # Create circuit
    qc = create_leggett_circuit_for_angle_six_barrier_NOP(phi_rad)
    ...
    for bitstring, count in counts.items():
        new_bitstring = bitstring[1:13]
        if new_bitstring not in new_counts:
            new_counts[new_bitstring] = count
        else:
            new_counts[new_bitstring] += count

    # Extract correlations for both +phi and -phi
    correlations_pos = extract_correlations_from_counts_six(new_counts, num_shots)
    ...
    print(f"\n  Unique bitstrings: {len(new_counts)}")
    ...
    return {
        'positive': {
            ...
            **result_pos,
            'job_id': job_id,
            'num_shots': num_shots,
            'unique_bitstrings': len(new_counts),
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_NOP_barrier_12qb.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_NOP_barrier_12qb.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_NOP_barrier_12qb.json')
