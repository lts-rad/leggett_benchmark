...
#!/usr/bin/env python3
"""
Leggett Inequality Test using IBM Quantum - Sequential Version with TOP 6 Pairs
...
Uses the absolute BEST 6 non-overlapping pairs from qubit pair scan (avg error 0.0019-0.0020).
"""
...
from leggett import create_leggett_circuit_for_angle_six, extract_correlations_from_counts_six, extract_correlations_from_counts, calc_leggett_for_angle
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    print(f"\n{'='*70}")
    print(f"Running complementary angles φ = ±{(phi_deg):.1f}° ({num_shots} shots, 12 qubits)")
    print(f"{'='*70}")
    ...
    if use_ibm:
        ...
        # Use best layout from qubit pair scan (avg error 0.0019-0.0020)
        # Pairs: (30,29), (86,87), (110,111), (13,14), (112,113), (153,152)
        best_layout = [30, 29, 86, 87, 110, 111, 13, 14, 112, 113, 153, 152]
        print(f"  Using best layout from scan: {best_layout}")
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_12qb_TOP6.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_12qb_TOP6.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_12qb_TOP6.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: Sequential Jobs with 12 Qubits (IBM Quantum)")
    print("USING ABSOLUTE TOP 6 PAIRS FROM QUBIT PAIR SCAN")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
    ...
    print(f"  Circuit size: 12 qubits (6 singlet pairs, testing ±φ simultaneously)")
    print(f"  Layout: TOP 6 from scan - pairs (30,29), (86,87), (110,111), (13,14), (112,113), (153,152)")
