...
For each angle φ, all 12 correlations are measured in one circuit.
...
def create_leggett_circuit_midcircuit(phi_rad):
    ...
    for phi in angles_rad:
        ...
        # 12 correlations for this angle
        measurement_pairs = [
            (a1, b1),
            (a1, b1_prime),
            (a2, b2),
...
def extract_correlations_from_midcircuit_counts(counts, num_shots, num_angles=1):
    ...
    Returns:
        If num_angles == 1: List of 12 correlations
        If num_angles > 1: List of lists, each with 12 correlations
    """
    total_correlations = num_angles * 12
    ...
    if num_angles == 1:
        # Return single list of 12 correlations
        return [correlations[0:12]]
    else:
        # Return list of lists
        angle_results = []
        for angle_idx in range(num_angles):
            start_idx = angle_idx * 12
            angle_corrs = correlations[start_idx:start_idx + 12]
...
def run_leggett_test_midcircuit(angles_deg, num_shots=1000, use_ibm=False,
    ...
    if use_ibm:
        ...
        # Extract counts from ALL classical registers (12 * num_angles)
        counts = {}
        total_cregs = len(angles_deg) * 12
...
if __name__ == "__main__":
    ...
    for angle in sweep:
        ...
        if USE_IBM_HARDWARE:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twelve_{BACKEND_NAME}_{timestamp}.json')
        elif USE_NOISE_MODEL:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twelve_noise_{args.noise_model}_{timestamp}.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twelve_ideal_sim_{timestamp}.json')
