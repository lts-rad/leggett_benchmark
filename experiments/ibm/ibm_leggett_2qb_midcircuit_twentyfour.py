...
#!/usr/bin/env python3
"""
Leggett Inequality Test  - Single Circuit with Mid-Circuit Measurements

This version creates a single circuit with 2 qubits that are reused N times
with mid-circuit measurements and barriers between each correlation measurement.
For each angle φ, all N correlations are measured in one circuit.
...
from leggett import poincare_to_angles, measure_polarization, extract_correlation_from_counts

def create_leggett_circuit_midcircuit(phi_rad):
    """
    Create a circuit with mid-circuit measurements for all N correlations.
    ...
    Returns:
        QuantumCircuit with 2 qubits and N*len(angles) classical registers
    """
...
def extract_correlations_from_midcircuit_counts(counts, num_shots, num_angles=1):
    ...
    Returns:
        If num_angles == 1: List of N correlations
        If num_angles > 1: List of lists, each with N correlations
    """
    total_correlations = num_angles * N
    ...
    if num_angles == 1:
        # Return single list of N correlations
        return [correlations[0:N]]
    else:
        # Return list of lists
        angle_results = []
        for angle_idx in range(num_angles):
            start_idx = angle_idx * N
            angle_corrs = correlations[start_idx:start_idx + N]
...
def run_leggett_test_midcircuit(angles_deg, num_shots=1000, use_ibm=False,
    ...
    print(f"  Total correlations: {len(angles_deg) * N}")
    print(f"{'='*70}")
    ...
    if use_ibm:
        ...
        # Extract counts from ALL classical registers (N * num_angles)
        counts = {}
        total_cregs = len(angles_deg) * N
...
if __name__ == "__main__":
    ...
    sweep = [-60, -45, -30, -25, -15, 15, 25, 30, 45, 60]
    ...
    for angle in sweep:
        ...
        if USE_IBM_HARDWARE:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twentyfour_{BACKEND_NAME}_{timestamp}.json')
        elif USE_NOISE_MODEL:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twentyfour_noise_{args.noise_model}_{timestamp}.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_midcircuit_twentyfour_ideal_sim_{timestamp}.json')
