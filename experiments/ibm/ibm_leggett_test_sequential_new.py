...
#!/usr/bin/env python3
"""
Leggett Inequality Test using IBM Quantum - Sequential Version (New Gate Scheme)
Based on arXiv:0801.2241v2

This version uses optimized measurement rotations:
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    print(f"Using optimized Rx/Ry measurement rotations")
    print(f"{'='*70}")

    # Create circuit with optimized gates
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_24qb_new.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_24qb_new.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_24qb_new.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: Sequential Jobs with 24 Qubits (New Gates)")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
    ...
    print(f"  Gate scheme: Optimized Rx/Ry (fewer gates than Rz/Ry)")
