...
#!/usr/bin/env python3
"""
Leggett Inequality Test using IBM Quantum - Sequential Version with 48 Qubits + REDUNDANCY
Based on arXiv:0801.2241v2

This version runs SEPARATE jobs for each angle to avoid large circuit decoherence.
Each job measures 12 correlations (6 for +φ, 6 for -φ) using 24 independent singlet pairs = 48 qubits total.
Each correlation is measured by 2 independent pairs for better statistics (redundancy).
"""
...
from leggett import create_leggett_circuit_for_angle_redundant, extract_correlations_from_counts_redundant, calc_leggett_for_angle
...
def run_single_angle(phi_deg, num_shots, use_ibm, backend_name, dry_run=False, enable_error_mitigation=True, use_noise_model=False):
    ...
    print(f"\n{'='*70}")
    print(f"Running complementary angles φ = ±{abs(phi_deg):.1f}° ({num_shots} shots, 48 qubits with redundancy)")
    print(f"{'='*70}")

    # Create circuit (measures both +phi and -phi, each correlation measured 2x for redundancy)
    qc = create_leggett_circuit_for_angle_redundant(phi_rad)
    ...
    if use_ibm:
        ...
        # Use best 24 non-overlapping pairs from qubit pair scan for 48-qubit redundant circuit
        best_layout = [67, 68, 29, 30, 54, 55, 93, 92, 109, 110, 2, 3, 13, 14, 112, 113, 153, 152, 75, 74, 94, 95, 107, 97, 86, 87, 125, 126, 20, 21, 51, 58, 101, 102, 35, 19, 76, 61, 80, 81, 142, 141, 17, 27, 99, 115, 105, 106]
        print(f"  Using best 48-qubit layout from scan (24 pairs): {len(best_layout)} qubits")
    ...
    # Extract correlations for both +phi and -phi (48 qubit circuit, each correlation measured 2x)
    correlations_pos, correlations_neg = extract_correlations_from_counts_redundant(counts, num_shots)
...
def main():
    ...
    if output_file is None:
        if use_ibm:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_sequential_48qb_BEST_LAYOUT.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_ibm_{backend_name}_NOISE_MODEL_48qb_BEST_LAYOUT.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_ibm_sequential_SIM_noiseless_48qb_BEST_LAYOUT.json')

    print("="*70)
    print("LEGGETT INEQUALITY TEST: Sequential Jobs with 48 Qubits + REDUNDANCY (IBM Quantum)")
    print("USING BEST 24 NON-OVERLAPPING PAIRS FROM QUBIT PAIR SCAN")
    print("="*70)
    print(f"\nConfiguration:")
    if use_ibm:
    ...
    print(f"  Circuit size: 48 qubits (24 singlet pairs, testing ±φ with 2x redundancy)")
    print(f"  Layout: BEST 24 pairs - first 12 pairs same as 24qb version + 12 more")
    print(f"  Redundancy: Each correlation measured by 2 independent pairs, then averaged")
    print(f"  Error mitigation: {'enabled' if enable_error_mitigation else 'disabled'}")
    print(f"  Dry run: {dry_run}")
    print(f"  Output file: {output_file}")

    # Each job tests BOTH +φ and -φ simultaneously, so only need positive angles
