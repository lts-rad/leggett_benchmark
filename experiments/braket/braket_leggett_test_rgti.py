...
def run_single_angle(phi_deg, num_shots, use_braket, device_arn, dry_run=False):
    ...
    if use_braket:
        ...
        # For Rigetti Ankaa-3: manually select 12 connected pairs from the topology
        # to avoid routing overhead. Map logical qubits to physical qubits with direct connectivity.
        initial_layout = [0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 15, 16, 17, 18, 22, 23, 24, 25, 29, 30, 31, 32, 37, 38]
        qc_transpiled = transpile(qc, backend=backend, optimization_level=1,
                                   initial_layout=initial_layout, routing_method='sabre')
...
def main():
    ...
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_rigetti_ankaa3_sequential.json')
