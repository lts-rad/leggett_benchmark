...
#!/usr/bin/env python3
"""
Leggett Inequality Test using Azure Quantum (Quantinuum H2) - Mid-Circuit Measurements

This version creates a single circuit with 2 qubits that are reused multiple times
with mid-circuit measurements and barriers between each correlation measurement.
For each angle φ, 6 correlations are measured in one circuit using qubit reuse.

Quantinuum H2 supports mid-circuit measurements, allowing qubit reuse.
"""
...
def run_leggett_test_midcircuit(angles_deg, num_shots=1000, use_ibm=False,
                                backend_name='', location='eastus', noise_model=None,
                                noise_backend_name=''):
    """
    ...
    if use_ibm:
        provider = AzureQuantumProvider(
            resource_id=backend_name,  # Will be Azure resource ID
            location="eastus"  # Default location, can be overridden
        )
        backend = provider.get_backend("quantinuum.qpu.h2-1")
        print(f"Using Azure Quantum backend: quantinuum.qpu.h2-1")
    elif noise_model:
    ...
    if use_ibm:
        # Transpile and run on Azure Quantum / Quantinuum H2
        print(f"\n  Transpiling for Quantinuum H2...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hardware', action='store_true', help='Run on Azure Quantum H2 hardware')
    parser.add_argument('--resource-id', type=str, required=True, help='Azure Quantum resource ID')
    parser.add_argument('--location', type=str, default='eastus', help='Azure location')
    parser.add_argument('--shots', type=int, default=1000, help='Number of shots per circuit')
    args = parser.parse_args()

    angles_deg = [-60, -45, -30, -25, -15, 15, 25, 30, 45, 60]  # Both positive and negative angles

    # Run test
    print("="*70)
    print("LEGGETT INEQUALITY TEST - MID-CIRCUIT MEASUREMENT MODE (Azure Quantum H2)")
    print("="*70)

    # Determine mode
    USE_IBM_HARDWARE = args.hardware  # Reusing variable name, but actually Azure
    USE_NOISE_MODEL = False  # Not supported for Azure
    BACKEND_NAME = args.resource_id
    ...
    if USE_IBM_HARDWARE:
        output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_azure_h2_{timestamp}.json')
    elif USE_NOISE_MODEL:
        output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_azure_h2_noise_{args.noise_model}_{timestamp}.json')
    else:
        output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_azure_h2_ideal_sim_{timestamp}.json')
