...
def run_single_angle(phi_deg, num_shots, use_azure, resource_id, location='eastus', dry_run=False, use_noise_model=False):
    ...
    if use_azure:
        ...
        # Use H2-1 emulator (56 qubits, other option: quantinuum.sim.h2-1sc for syntax check)
        # For real hardware, you need to add the Quantinuum provider to your workspace
        backend = provider.get_backend("rigetti.qpu.cepheus-1-36q")
        ...
        while True:
            ...
            if status in ['DONE', 'COMPLETED', 'Succeeded', 'ERROR', 'CANCELLED', 'Failed']:
...
def main():
    ...
    if output_file is None:
        if use_azure:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_azure_sim_h2-1_sequential_24qb.json')
        elif use_noise_model:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', f'leggett_results_azure_h2_NOISE_MODEL_24qb.json')
        else:
            output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'leggett_results_azure_h2_SIM_noiseless_24qb.json')
