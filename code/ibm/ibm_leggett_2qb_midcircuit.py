...
This version creates a single circuit with 2 qubits that are reused 6 times
with mid-circuit measurements and barriers between each correlation measurement.
For each angle φ, all 6 correlations are measured in one circuit.
...
def create_leggett_circuit_midcircuit(phi_rad):
    """
    Create a circuit with mid-circuit measurements for all 6 correlations.
    ...
    Returns:
        QuantumCircuit with 2 qubits and 6*len(angles) classical registers
    """
...
if __name__ == "__main__":
    ...
    for angle in sweep:
        ...
        if USE_IBM_HARDWARE:
            output_file = f'leggett_results_ibm_midcircuit_{BACKEND_NAME}_{timestamp}.json'
        elif USE_NOISE_MODEL:
            output_file = f'leggett_results_ibm_midcircuit_noise_{args.noise_model}_{timestamp}.json'
        else:
            output_file = f'leggett_results_ibm_midcircuit_ideal_sim_{timestamp}.json'
