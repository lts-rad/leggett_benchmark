...
#!/usr/bin/env python3
"""
Create radar plot for OPTIMIZED layout hardware results from IBM Pittsburgh.
"""
...
# Plot data
ax.plot(angles, corr_theory_plot, 'o--', linewidth=2, label=f'QM Ideal (L₃={data["positive"]["L3_theory"]:.4f})',
        color='royalblue', markersize=8, alpha=0.7)
ax.plot(angles, corr_bound_plot, ':', linewidth=3, label=f'Leggett Bound (L₃={bound:.4f})',
        color='gray', alpha=0.7)
ax.plot(angles, corr_pos_plot, 'o-', linewidth=2.5, label=f'φ = +{phi_deg}° (L₃={data["positive"]["L3"]:.4f})',
        color='green', markersize=10)
ax.plot(angles, corr_neg_plot, 's-', linewidth=2.5, label=f'φ = -{phi_deg}° (L₃={data["negative"]["L3"]:.4f})',
...
ax.set_yticks([-1.04, -1.02, -1.00, -0.98, -0.96, -0.94, -0.92, -0.90, -0.88, -0.86])
ax.set_yticklabels(['-1.04', '-1.02', '-1.00', '-0.98', '-0.96', '-0.94', '-0.92', '-0.90', '-0.88', '-0.86'], size=10)
...
title += f'QM Theory: L₃ = {data["positive"]["L3_theory"]:.4f} | Leggett Bound: L₃ ≤ {bound:.4f}\n'
title += f'IBM Pittsburgh - HARDWARE - Optimized Layout ({data["num_shots"]} shots)'
...
textstr += f'Margin (+{phi_deg}°): {data["positive"]["L3"] - bound:+.4f}\n'
textstr += f'Margin (-{phi_deg}°): {data["negative"]["L3"] - bound:+.4f}\n'
textstr += f'Job ID: {data["job_id"]}'
...
# Save
output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'plots', 'radar_optimized_hardware_pittsburgh.png')
