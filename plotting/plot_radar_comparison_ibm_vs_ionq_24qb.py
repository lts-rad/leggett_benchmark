#!/usr/bin/env python3
"""
Create side-by-side radar plots comparing IBM Pittsburgh 24qb vs IonQ Forte 24qb.
"""
...
title1 += f'Margin (+): {ibm_pos["L3"] - bound:+.4f} | Margin (-): {ibm_neg["L3"] - bound:+.4f}'
...
title2 += f'Margin (+): {ionq_result["L3_pos"] - bound:+.4f} | Margin (-): {ionq_result["L3_neg"] - bound:+.4f}'
...
# Overall title
fig.suptitle(f'Leggett Inequality Test Comparison: IBM vs IonQ (φ = ±{phi_deg}°)',
             fontsize=16, weight='bold', y=0.98)

plt.tight_layout(rect=[0, 0, 1, 0.96])

# Save
output_file = f'radar_comparison_ibm_vs_ionq_24qb_phi_{phi_deg}.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f'Saved to {output_file}')

print(f'\n=== Comparison Summary (φ = ±{phi_deg}°) ===')
print(f'\nIBM Pittsburgh 24qb:')
print(f'  L₃(+{phi_deg}°) = {ibm_pos["L3"]:.4f} | Margin: {(ibm_pos["L3"] - bound) / bound * 100:+.2f}%')
print(f'  L₃(-{phi_deg}°) = {ibm_neg["L3"]:.4f} | Margin: {(ibm_neg["L3"] - bound) / bound * 100:+.2f}%')
print(f'\nIonQ Forte 24qb:')
print(f'  L₃(+{phi_deg}°) = {ionq_result["L3_pos"]:.4f} | Margin: {(ionq_result["L3_pos"] - bound) / bound * 100:+.2f}%')
print(f'  L₃(-{phi_deg}°) = {ionq_result["L3_neg"]:.4f} | Margin: {(ionq_result["L3_neg"] - bound) / bound * 100:+.2f}%')
