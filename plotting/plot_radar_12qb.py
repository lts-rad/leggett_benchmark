...
#!/usr/bin/env python3
"""
Create radar plot for Leggett correlation measurements from 12qb best layout.
"""
...
# Match based on phi_rad sign since phi_deg format varies
result_pos = None
result_neg = None

for r in results:
    if abs(abs(r['phi_rad']) - 0.5236) < 0.01:  # phi ≈ ±30° (0.5236 rad)
        if r['phi_rad'] > 0:
            result_pos = r
        else:
            result_neg = r
...
title += f'IBM Pittsburgh - 12 Qubits (6 pairs) - Best Layout'
...
# Save
output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'plots', 'pittsburgh_12qb_BEST_LAYOUT_phi_30.png')
