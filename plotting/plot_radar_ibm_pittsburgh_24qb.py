...
#!/usr/bin/env python3
"""
Radar plots for IBM Pittsburgh 24qb sequential results.
...
# Physical qubit pairs from transpiler (based on dry-run output)
# These are the default transpiler selections for IBM Pittsburgh
# Pairs 0-5 are +phi, pairs 6-11 are -phi
pairs_pos = ['2-3', '5-4', '0-1', '7-6', '21-22', '25-24']
pairs_neg = ['9-8', '45-37', '23-16', '27-17', '87-86', '93-79']

# Load data
with open('leggett_results_ibm_ibm_pittsburgh_sequential_24qb.json', 'r') as f:
    data = json.load(f)
...
def create_radar_subplot(ax, phi_deg):
    ...
    # Title
    ax.set_title(f'phi = +/-{phi_deg}deg', size=14, weight='bold', pad=15, color=color_text)

    # Legend
    legend = ax.legend(
        [qm_line, leggett_line, pos_line, neg_line],
        [f'QM Theory (L3={L3_theory:.3f})',
         f'Leggett Bound ({bound:.3f})',
         f'+{phi_deg}deg (L3={L3_pos:.3f})',
         f'-{phi_deg}deg (L3={L3_neg:.3f})'],
...
# Main title
fig.suptitle('IBM Pittsburgh Best-24 Layout (Sequential 24qb)\n1000 Shots',
             size=18, weight='bold', color=color_text, y=0.98)

plt.tight_layout()

output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'plots', 'radar_ibm_pittsburgh_sequential_24qb.png')
